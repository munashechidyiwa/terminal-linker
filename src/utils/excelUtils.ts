...
/**
 * Generate and download an Excel report for filtered terminals
 * @param terminals List of filtered terminals to include in the report
 * @param reportType Type of report being generated
 */
export const downloadFilteredTerminalsReport = async (
  terminals: Terminal[],
  reportType: 'filtered'
): Promise<void> => {
  // Format data for Excel - same as downloadTerminalsReport
  const data = terminals.map(terminal => ({
    'Merchant Name': terminal.name,
    'Terminal ID': terminal.terminalId,
    'Serial Number': terminal.serialNumber,
  ...
  const worksheet = utils.json_to_sheet(data);
  const workbook = utils.book_new();
  utils.book_append_sheet(workbook, worksheet, 'Filtered Terminals');
